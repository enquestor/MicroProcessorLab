...
void display_three(int a, int b, int c)
{
	int i, j = 1;
	int da = digits(a), db = digits(b), dc = digits(c);
	for(i=1; i <= dc; i++)
	{
		max7219_send(j++, c%10);
		c/=10;
...
void keypad_scan()
{
	...
	while(1)
	{
		...
		else if(pressed == 1)
		{
			...
			display_three(rm, total-rm, total);
	}
}
